...
const VideoPromotionCard = memo(({ media, loading, isHomepage }: { media: AuthPromotionMedia[]; loading: boolean; isHomepage: boolean }) => {
  ...
  return <Frame label="Homepage promotion slot 1: video playlist" index={0}><div className="absolute inset-0 flex items-center justify-center bg-black" onClick={(e) => { e.preventDefault(); e.stopPropagation(); const v = videoRef.current; if (!v) return; if (v.paused) { userPausedRef.current = false; v.muted = mutedRef.current; v.play().catch(() => {}); } else { userPausedRef.current = true; v.pause(); } }} style={{ cursor: 'pointer' }}><video ref={videoRef} key={current.id} className="h-full w-full object-contain brightness-[1.08]" style={{ pointerEvents: 'none' }} src={current.media_url} muted={muted} loop={playable.length === 1} playsInline preload="metadata" controlsList="nodownload noplaybackrate nofullscreen" disablePictureInPicture onContextMenu={(event) => event.preventDefault()} onEnded={advance} onError={failedCurrent} aria-label="Vowza promotional video" /></div><div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-black/35 via-transparent to-black/10" /></Frame>;
});
