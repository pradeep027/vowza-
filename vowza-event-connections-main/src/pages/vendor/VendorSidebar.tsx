        ...
        {NAV_ITEMS.map(item => {
          ...
          return (
            <NavLink
              ...
              title={collapsed ? (item.to === '/vendor/packages' ? isWaterSupplier(provider) ? 'Water Products' : isPhotographer(provider) ? 'Photography Packages' : isCaterer(provider) ? 'Catering Packages' : isVideographer(provider) ? 'Videography Packages' : item.label : item.label) : undefined}
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  {!collapsed && (
                    <>
                      <span className="truncate">{item.to === '/vendor/packages' ? isWaterSupplier(provider) ? 'Water Products' : isPhotographer(provider) ? 'Photography Packages' : isCaterer(provider) ? 'Catering Packages' : isVideographer(provider) ? 'Videography Packages' : item.label : item.label}</span>
                      {count > 0 && (
