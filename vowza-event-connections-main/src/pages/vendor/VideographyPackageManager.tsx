...
import {
  ...
  ChevronRight, ChevronLeft, Upload, Video,
} from 'lucide-react';
...
/* ─── Constants ─────────────────────────────────────────────────────────────── */
const EVENT_TYPES = [
  'Pre-Wedding Shoot','Wedding Cinematic','Reception','Engagement',
  'Haldi & Mehendi','Birthday','Corporate Event','Music Video',
  'Product Shoot','Other',
];
const COVERAGE_DURATIONS = ['2 Hours','4 Hours','6 Hours','8 Hours','12 Hours','Full Day','Multi-Day'];
const COVERAGE_INCLUDES = [
  'Cinematic Wedding Film','Traditional Video','Highlight Reel','Teaser Video',
  'Instagram Reel','YouTube Edit','Live Streaming','Couple Shoot',
  'Ceremony Coverage','Reception Coverage','Bride Preparation','Groom Preparation',
  'Candid Video','Slow Motion Shots','4K Recording','Multi Camera Setup',
];
const DELIVERABLES = [
  'Full Wedding Film','Highlight Film','Trailer','Instagram Reels','Teaser',
  'Short Clips','RAW Footage','Edited Footage','USB Delivery','Cloud Download',
  'Google Drive Link',
];
const DELIVERY_TIMES = ['7 Days','15 Days','30 Days','45 Days','60 Days'];
const EQUIPMENT_OPTIONS = [
  'DSLR','Mirrorless','Cinema Camera','Gimbal','Crane','Slider','Tripod',
  'LED Lights','Wireless Microphones','Audio Recorder','Steadicam',
  'Telephoto Lens','Wide Lens',
];

const STEP_LABELS = ['Basic Info', 'Pricing', 'Coverage', 'Deliverables', 'Team & Equipment', 'Add-ons'];
...
const blank = (): Draft => ({
  ...
  package_price: '', travel_charges: '', extra_coverage_cost: '',
  advance_percentage: '20',
  coverage_hours: '4 Hours', num_cameras: '1',
  coverage_includes: [], deliverables: [], delivery_time: '15 Days',
  equipment: [],
  team_videographers: '1', team_assistants: '0', team_editor: '1',
  addons: [],
...
export default function VideographyPackageManager({ provider }: { provider: any }) {
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: ['videography-packages', provider.id] });

  useEffect(() => {
    const channel = supabase
      .channel(`videography-packages-${provider.id}`)
      .on('postgres_changes', {
        event: '*', schema: 'public', table: 'videography_packages',
  ...
  const edit = async (pkg: any) => {
    ...
    setDraft({
      ...
      extra_coverage_cost: String(pkg.extra_coverage_cost ?? ''),
      advance_percentage: String(pkg.advance_percentage ?? '20'),
      coverage_hours: pkg.coverage_hours || '4 Hours',
      num_cameras: String(pkg.num_cameras ?? '1'),
      coverage_includes: pkg.coverage_includes ?? [],
      deliverables: pkg.deliverables ?? [],
      delivery_time: pkg.delivery_time || '15 Days',
  ...
  const save = async () => {
    ...
    try {
      const payload: any = {
        ...
        extra_coverage_cost: draft.extra_coverage_cost ? Number(draft.extra_coverage_cost) : null,
        advance_percentage: draft.advance_percentage ? Number(draft.advance_percentage) : 20,
        coverage_hours: draft.coverage_hours || null,
        num_cameras: draft.num_cameras ? Number(draft.num_cameras) : 1,
        coverage_includes: draft.coverage_includes,
        deliverables: draft.deliverables,
        delivery_time: draft.delivery_time || null,
        equipment: draft.equipment,
        team_videographers: draft.team_videographers ? Number(draft.team_videographers) : 1,
        team_assistants: draft.team_assistants ? Number(draft.team_assistants) : 0,
        team_editor: draft.team_editor ? Number(draft.team_editor) : 1,
      };
  ...
  const renderStep = () => {
    ...
    switch (step) {
      ...
      case 4: return <StepDeliverables draft={draft} setDraft={setDraft} ChipSelect={ChipSelect} />;
      case 5: return <StepTeamEquipment draft={draft} setDraft={setDraft} ChipSelect={ChipSelect} />;
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="block sm:col-span-2">
            ...
            <input className={inputClass} value={draft.name}
              ...
              placeholder="e.g. Premium Wedding Cinematic Package" />
          </label>
          ...
          <label className="block sm:col-span-2">
            ...
            <textarea className={`${inputClass} min-h-[80px] resize-y`} value={draft.description}
              ...
              placeholder="Describe what makes this videography package special..." />
          </label>
          ...
          <div className="sm:col-span-2">
            ...
            <p className="text-xs text-stone-500 mb-2">Upload sample work, behind-the-scenes, and production stills (max 8 images)</p>
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-[#eadfcf] bg-[#fffdfa] p-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="block">
            <span className="text-sm font-semibold text-[#4b1d2b]">Extra Coverage Cost / Hour</span>
            <div className="relative">
              <span className="absolute left-3.5 top-2.5 text-sm text-stone-500">₹</span>
              <input className={`${inputClass} pl-7`} type="number" min="0"
                value={draft.extra_coverage_cost}
                onChange={e => setDraft({ ...draft, extra_coverage_cost: e.target.value })}
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-[#eadfcf] bg-[#fffdfa] p-5 space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="block">
            <span className="text-sm font-semibold text-[#4b1d2b]">Number of Cameras</span>
            <input className={inputClass} type="number" min="1" max="5"
              value={draft.num_cameras}
              onChange={e => setDraft({ ...draft, num_cameras: e.target.value })}
              placeholder="1" />
          </label>
        </div>
...
function StepDeliverables({ draft, setDraft, ChipSelect }: { draft: Draft; setDraft: (d: Draft) => void; ChipSelect: any }) {
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-[#eadfcf] bg-[#fffdfa] p-5 space-y-5">
        <h3 className="text-base font-bold text-[#62132d]">Deliverables & Delivery</h3>
...
function StepTeamEquipment({ draft, setDraft, ChipSelect }: { draft: Draft; setDraft: (d: Draft) => void; ChipSelect: any }) {
  return (
    <div className="space-y-4">
      <div className="rounded-2xl border border-[#eadfcf] bg-[#fffdfa] p-5 space-y-5">
        <h3 className="text-base font-bold text-[#62132d]">Team & Equipment</h3>
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          ...
          <label className="block">
            <span className="text-sm font-semibold text-[#4b1d2b]">Editors</span>
            <input className={inputClass} type="number" min="0" max="10"
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-[#eadfcf] bg-[#fffdfa] p-5">
        ...
        {draft.addons.length === 0 ? (
          <p className="text-center text-sm text-stone-400 py-4">No add-ons yet. Add optional extras like Extended Edit, Same Day Teaser, Drone Coverage, etc.</p>
        ) : (
      ...
      <div className="rounded-2xl border border-[#eadfcf] bg-[#fffdfa] p-5">
        ...
        <div className="overflow-hidden rounded-2xl border border-[#eadfcf] bg-white shadow-sm">
          {draft.cover_file || draft.cover_url ? (
            ...
            <div className="flex h-24 items-center justify-center bg-gradient-to-br from-rose-50 to-purple-50">
          ...
          <div className="p-4">
            ...
            <div className="mt-1.5 flex items-center gap-2 text-xs text-stone-500">
              <Video className="h-3.5 w-3.5" />
              {draft.event_type || 'Videography'} · {draft.coverage_hours}
            </div>
            {draft.coverage_includes.length > 0 && (
              <div className="mt-2.5 flex flex-wrap gap-1">
                {draft.coverage_includes.slice(0, 4).map(c => (
                  <span key={c} className="rounded-full bg-[#8b1538]/8 px-2 py-0.5 text-[11px] font-medium text-[#8b1538]">{c}</span>
                ))}
                {draft.coverage_includes.length > 4 && (
                  <span className="rounded-full bg-stone-100 px-2 py-0.5 text-[11px] font-medium text-stone-500">+{draft.coverage_includes.length - 4}</span>
                )}
              </div>
            )}
            ...
            {(Number(draft.team_videographers) > 0 || Number(draft.team_editor) > 0) && (
              <div className="mt-3 border-t border-stone-100 pt-3">
                <p className="text-xs font-semibold text-stone-600 mb-1">Team:</p>
                <p className="text-xs text-stone-500">
                  {draft.team_videographers} Videographer{Number(draft.team_videographers) > 1 ? 's' : ''}
                  {Number(draft.team_assistants) > 0 && ` · ${draft.team_assistants} Assistant${Number(draft.team_assistants) > 1 ? 's' : ''}`}
                  {Number(draft.team_editor) > 0 && ` · ${draft.team_editor} Editor${Number(draft.team_editor) > 1 ? 's' : ''}`}
                </p>
              </div>
            )}
