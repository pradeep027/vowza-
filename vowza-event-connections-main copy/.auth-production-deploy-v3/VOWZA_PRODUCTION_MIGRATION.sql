...
-- Drop and recreate enum for provider professions
...
-- ============================================
-- PART 3: ENHANCED AUTH TABLES (with IF NOT EXISTS)
-- ============================================
...
-- ============================================
-- PART 4: ADDITIONAL TABLES (with IF NOT EXISTS)
-- ============================================
...
-- ============================================
-- PART 6: FUNCTIONS (with CREATE OR REPLACE for idempotency)
-- ============================================
...
-- ============================================
-- PART 8: ENABLE RLS (with check for idempotency)
-- ============================================
...
-- Note: In Supabase, storage buckets are created via API or dashboard
-- We use INSERT with ON CONFLICT for idempotency
DO $$
BEGIN
...
-- Storage policies depend on buckets existing
DO $$
BEGIN
...
-- ============================================
-- PART 13: ENABLE REALTIME (with check for idempotency)
-- ============================================
...
DECLARE
  v_table TEXT;
BEGIN
  FOREACH v_table IN ARRAY ARRAY['bookings', 'notifications', 'messages']
  LOOP
    BEGIN
      IF NOT EXISTS (
...
-- ============================================
-- PART 14: AI CONVERSATIONS (persistent chat history for Vowza Planner)
-- ============================================
...
-- ============================================
-- PART 15: BOOKING AVAILABILITY FUNCTIONS
-- ============================================
...
-- check_artist_availability: prevent double bookings with advisory lock
  ...
  IF p_event_time IS NULL THEN
    ...
    IF v_count > 0 THEN
      RETURN jsonb_build_object('available', FALSE,
        'reason', 'Artist already has ' || v_count || ' booking(s) on this date.');
    END IF;
  ELSE
    SELECT EXISTS (
    ...
    IF v_conflict THEN
      RETURN jsonb_build_object('available', FALSE,
        'reason', 'Artist is already booked during this time slot.');
    END IF;
  END IF;
...
-- Fix provider_availability RLS to allow artists to UPDATE/DELETE their own blocks
DROP POLICY IF EXISTS "Providers can manage own availability" ON public.provider_availability;
