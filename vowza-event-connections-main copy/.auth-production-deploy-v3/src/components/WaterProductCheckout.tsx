...
export default function WaterProductCheckout({ payload }: { payload: Payload }) {
  ...
  const placeOrder = async () => {
    ...
    setLoading(true);
    const { data, error } = await supabase.rpc('create_water_product_order' as any, { p_provider_id: payload.providerId, p_items: payload.lines.map(line => ({ productId: line.productId, variantId: line.variantId, quantity: line.quantity })), p_delivery_address: address.trim(), p_delivery_lat: Number(latitude), p_delivery_lng: Number(longitude), p_delivery_date: date, p_delivery_time_slot: slot, p_special_instructions: instructions.trim() || null });
    setLoading(false);
