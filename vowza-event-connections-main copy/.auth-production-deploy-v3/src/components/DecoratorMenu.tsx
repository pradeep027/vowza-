...
import { Palette, Check, ChevronDown, ChevronUp, MapPin, Clock, Calendar, X, ChevronLeft, ChevronRight, Loader2, AlertCircle, ShoppingCart } from 'lucide-react';
...
export default function DecoratorMenu({ provider, profile }: { provider: any; profile: any }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const channel = supabase.channel(`public-decorator-${provider.id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'decorator_packages', filter: `provider_id=eq.${provider.id}` }, () => qc.invalidateQueries({ queryKey: ['public-decorator-packages', provider.id] }))
      .subscribe();
  ...
  const handleBook = (pkg: any) => {
    ...
    setBookingAddons(pkg.decorator_addons ?? []);
    setBookingPkg(pkg);
  };

  if (isLoading) return <div className="h-48 animate-pulse rounded-2xl bg-muted" />;
  if (!packages.length) return <div className="rounded-2xl border border-border/60 bg-surface-1 p-10 text-center text-sm text-muted-foreground">This decorator has not published any packages yet.</div>;

  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-xl font-bold">Decoration Packages</h2>
        <p className="text-sm text-muted-foreground">Browse decoration packages and book your event.</p>
      </div>
      <div className="grid gap-5 md:grid-cols-2">
        {packages.map((pkg: any) => {
          const isOpen = expanded[pkg.id];
          const gallery = pkg.decorator_gallery ?? [];
          const cover = gallery.find((g: any) => g.is_cover);
          const videos = gallery.filter((g: any) => g.media_type === 'video');
          const addons = pkg.decorator_addons ?? [];
          ...
          return (
            <div key={pkg.id} className="overflow-hidden rounded-2xl border border-border/60 bg-white shadow-sm transition hover:shadow-md">
              <div className="relative h-44 sm:h-52 bg-gradient-to-br from-rose-50 to-amber-50 overflow-hidden">
                {cover ? (
                  <img src={cover.public_url} alt={pkg.name} className="w-full h-full object-cover" loading="lazy" />
                ) : (
                  <div className="flex h-full items-center justify-center"><Palette className="h-12 w-12 text-rose-400/40" /></div>
                )}
              </div>
              <div className="p-5">
                ...
                {pkg.package_type && (
                  <div className="mt-2">
                    <span className="inline-flex items-center gap-1 rounded-full bg-[#f4d58d]/30 border border-[#f4d58d]/50 px-2.5 py-0.5 text-[11px] font-medium text-[#62132d]">
                      <Palette className="h-3 w-3" />{pkg.package_type}
                    </span>
                  </div>
                )}
                ...
                {(pkg.inclusions ?? []).length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-1">
                    {pkg.inclusions.slice(0, 5).map((inc: string) => (
                      <span key={inc} className="inline-flex items-center rounded-full bg-rose-50 border border-rose-100 px-2 py-0.5 text-[11px] text-rose-700">
                        {inc}
                      </span>
                    ))}
                    {pkg.inclusions.length > 5 && <span className="text-[11px] text-muted-foreground">+{pkg.inclusions.length - 5} more</span>}
                  </div>
                )}

                {/* Themes */}
                {(pkg.themes_available ?? []).length > 0 && (
                  <div className="mt-2 flex flex-wrap gap-1">
                    {pkg.themes_available.slice(0, 4).map((t: string) => (
                      <span key={t} className="inline-flex items-center rounded-full bg-amber-50 border border-amber-100 px-2 py-0.5 text-[11px] text-amber-700">
                        {t}
                      </span>
                    ))}
                    {pkg.themes_available.length > 4 && <span className="text-[11px] text-muted-foreground">+{pkg.themes_available.length - 4} more</span>}
                  </div>
                )}
                  ...
                  <div className="mt-3 space-y-3">
                    ...
                    {(pkg.inclusions ?? []).length > 0 && (
                      <div className="rounded-xl bg-secondary/50 p-3">
                        <p className="text-[10px] font-bold uppercase tracking-wide text-[#62132d] mb-1">All Inclusions</p>
                        <div className="flex flex-wrap gap-1">
                          {pkg.inclusions.map((inc: string) => (
                            <span key={inc} className="rounded-full border border-[#e7d9c4] px-2 py-0.5 text-xs text-stone-600">{inc}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    ...
                    {(pkg.themes_available ?? []).length > 0 && (
                      <div className="rounded-xl bg-amber-50/60 border border-amber-100 p-3">
                        <p className="text-[10px] font-bold uppercase tracking-wide text-amber-700 mb-1">Available Themes</p>
                        <div className="flex flex-wrap gap-1">
                          {pkg.themes_available.map((t: string) => (
                            <span key={t} className="rounded-full border border-amber-200 px-2 py-0.5 text-xs text-amber-800">{t}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    ...
                    {/* Venue types */}
                    {(pkg.venue_types ?? []).length > 0 && (
                      <div className="rounded-xl bg-blue-50/60 border border-blue-100 p-3">
                        <p className="text-[10px] font-bold uppercase tracking-wide text-blue-700 mb-1">Venue Types</p>
                        <div className="flex flex-wrap gap-1">
                          {pkg.venue_types.map((v: string) => (
                            <span key={v} className="rounded-full border border-blue-200 px-2 py-0.5 text-xs text-blue-800">{v}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    ...
                    <div className="flex gap-3 text-xs text-muted-foreground">
                      {pkg.setup_time && <span className="flex items-center gap-1"><Clock className="h-3 w-3" />Setup: {pkg.setup_time}</span>}
                      {pkg.teardown_time ? <span>Teardown: {pkg.teardown_time}</span> : pkg.teardown_included && <span className="text-emerald-600 font-medium">Teardown Included</span>}
                    </div>

                    {/* Videos */}
                    {videos.length > 0 && (
                      <div className="rounded-xl bg-rose-50/60 border border-rose-100 p-3">
                        <p className="text-[10px] font-bold uppercase tracking-wide text-rose-700 mb-2">Decoration Videos</p>
                        <div className="grid grid-cols-2 gap-2">
                          {videos.map((v: any) => (
                            <div key={v.id} className="rounded-xl overflow-hidden border border-rose-200 aspect-video bg-black">
                ...
                <button
                  onClick={() => { if (!user) { toast.error('Please log in'); return nav('/auth'); } addToCart({ packageId: pkg.id, providerId: provider.id, providerName: profile?.full_name || 'Artist', category: 'decorator', packageTable: 'decorator_packages', bookingTable: 'decorator_bookings', packageName: pkg.name, price: Number(pkg.package_price || pkg.price || 0), duration: undefined, imageUrl: (pkg.decorator_gallery ?? []).find((g: any) => g.is_cover)?.public_url || undefined }); }}
                  disabled={isInCart(pkg.id, 'decorator')}
                ...
                >
                  <ShoppingCart className="h-3 w-3" />{isInCart(pkg.id, 'decorator') ? 'In Cart' : 'Add to Cart'}
                </button>
              </div>
            </div>
          );
      ...
      <ScopedCartBar providerId={provider.id} category="decorator" />
      {/* Booking Modal */}
      {bookingPkg && (
        <DecoratorBookingModal
          isOpen={!!bookingPkg}
...
/* ─── Decorator Booking Modal ─────────────────────────────────────────────── */
function DecoratorBookingModal({ isOpen, onClose, pkg, provider, addons }: { isOpen: boolean; onClose: () => void; pkg: any; provider: any; addons: any[] }) {
  const { user } = useAuth();
  const nav = useNavigate();
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setLocation(emptyLocationData); setVenueType('');
      setSelectedTheme(''); setSelectedAddonIds([]);
      setSpecialRequests(''); setColorPreferences('');
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { data: booking, error } = await supabase.from('decorator_bookings' as any).insert({
        ...
        theme_preference: selectedTheme || null,
        special_instructions: [colorPreferences && `Colors: ${colorPreferences}`, specialRequests].filter(Boolean).join('\n') || null,
      ...
      sessionStorage.setItem('vowza_booking_success', JSON.stringify({
        ...
        artistName: provider.business_name || provider.contact_person || 'Decorator',
        eventDate, eventTime,
        venue: location.venue_name || location.locality || 'TBD',
        city: location.town_city || '',
        amount: total,
        advanceAmount, remainingBalance: remaining,
        eventType: pkg.package_type || 'Decoration',
        status: 'pending',
      }));

      toast.success('Decoration booking request sent!');
      onClose();
      nav('/booking-success');
    } catch (err: any) {
  ...
  const STEPS = ['Package', 'Event', 'Theme', 'Add-ons', 'Special Requests', 'Confirm'];
    ...
    <div className="fixed inset-0 z-[80] overflow-y-auto bg-[#250914]/65 p-3 backdrop-blur-sm sm:p-6">
      <div className="mx-auto my-3 max-w-2xl overflow-hidden rounded-[24px] bg-[#fffaf3] shadow-2xl">
        <header className="flex items-start justify-between gap-4 bg-[#70102d] px-5 py-5 sm:px-7">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#f4d58d]">Vowza Decorator</p>
        ...
        <div className="p-5 sm:p-7 max-h-[60vh] overflow-y-auto">
          ...
          {step === 1 && (
            <div className="rounded-2xl border border-[#eadfcf] bg-white p-5">
              ...
              {pkg.package_type && <p className="mt-1 text-xs text-muted-foreground"><Palette className="inline h-3 w-3 mr-1" />Type: {pkg.package_type}</p>}
              {(pkg.inclusions ?? []).length > 0 && (
                <div className="mt-3">
                  <p className="text-xs font-semibold text-stone-600 mb-1">Inclusions:</p>
                  <div className="flex flex-wrap gap-1">
                    {pkg.inclusions.map((inc: string) => (
                      <span key={inc} className="rounded-full bg-[#8b1538]/8 px-2 py-0.5 text-[11px] text-[#8b1538]">{inc}</span>
                    ))}
                  </div>
                </div>
              )}
              {(pkg.themes_available ?? []).length > 0 && (
                <div className="mt-3">
                  <p className="text-xs font-semibold text-stone-600 mb-1">Available Themes:</p>
                  <div className="flex flex-wrap gap-1">
                    {pkg.themes_available.map((t: string) => (
                      <span key={t} className="rounded-full bg-[#f4d58d]/30 px-2 py-0.5 text-[11px] text-[#62132d]">{t}</span>
                    ))}
                  </div>
                </div>
              )}
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div className="rounded-2xl border border-[#eadfcf] bg-white p-5 space-y-4">
                ...
                <label className="block">
                  <span className="text-sm font-semibold text-[#4b1d2b]">Venue Type</span>
                  <select className={inputClass} value={venueType}
                    onChange={e => setVenueType(e.target.value)}>
                    <option value="">Select venue type</option>
                    {['Indoor', 'Outdoor', 'Both', 'Home', 'Hall', 'Farm', 'Resort', 'Temple', 'Beach'].map(v => (
          ...
          {step === 3 && (
            <div className="space-y-4">
              <div className="rounded-2xl border border-[#eadfcf] bg-white p-5 space-y-4">
                <h3 className="font-bold text-[#3d1924]">Select Theme</h3>
                <p className="text-xs text-stone-500">Choose your preferred decoration theme for this package.</p>
                {(pkg.themes_available ?? []).length > 0 ? (
                  <div className="flex flex-wrap gap-2">
                    {pkg.themes_available.map((theme: string) => (
                      <button key={theme} type="button"
                        onClick={() => setSelectedTheme(theme)}
                        className={`rounded-full border px-4 py-2 text-sm font-medium transition ${
                          selectedTheme === theme
                            ? 'border-[#8b1538] bg-[#8b1538] text-white shadow-md'
                            : 'border-[#e7d9c4] text-stone-600 hover:border-[#c99b43] hover:bg-[#f4d58d]/10'
                        }`}>
                        {theme}
                      </button>
                    ))}
                  ...
                  <p className="text-sm text-stone-400">No predefined themes for this package. Add your preferences in Special Requests.</p>
                )}
                {selectedTheme && (
                  <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-3">
                    <p className="text-xs font-semibold text-emerald-700 flex items-center gap-1">
                      <Check className="h-3.5 w-3.5" />Selected: {selectedTheme}
                    </p>
                  </div>
                )}
              </div>
          ...
          {step === 4 && (
            <div className="space-y-4">
              <div className="rounded-2xl border border-[#eadfcf] bg-white p-5 space-y-4">
                ...
                <p className="text-xs text-stone-500">Select optional extras to enhance your decoration.</p>
                {addons.length > 0 ? (
          ...
          {step === 5 && (
            <div className="space-y-4">
              <div className="rounded-2xl border border-[#eadfcf] bg-white p-5 space-y-4">
                <h3 className="font-bold text-[#3d1924]">Special Requests</h3>
                <label className="block">
                  <span className="text-sm font-semibold text-[#4b1d2b]">Color Preferences</span>
                  <input className={inputClass} value={colorPreferences}
                    onChange={e => setColorPreferences(e.target.value)}
                    placeholder="e.g. Red & Gold, Pastel Pink, White & Silver" />
                </label>
                <label className="block">
                  ...
                  <textarea className={`${inputClass} min-h-[100px] resize-y`} value={specialRequests}
                    ...
                    placeholder="Any specific requirements, inspirations, or references you'd like the decorator to know about..." />
                </label>
              </div>
            </div>
          )}
            ...
            <div className="space-y-4">
              <div className="rounded-2xl border border-[#eadfcf] bg-white p-5 space-y-3">
                ...
                <div className="space-y-2 text-sm">
                  ...
                  {selectedTheme && <div className="flex justify-between"><span className="text-stone-600">Theme</span><span className="font-medium">{selectedTheme}</span></div>}
                  ...
                  {venueType && <div className="flex justify-between"><span className="text-stone-600">Venue Type</span><span className="font-medium">{venueType}</span></div>}
                </div>
                ...
                <label className="flex items-start gap-3 cursor-pointer mt-4 rounded-xl border border-[#eadfcf] p-3">
                  ...
                  <span className="text-xs text-stone-600">
                    I agree to the booking terms. The decorator will review my request and confirm availability. Advance payment will be required after confirmation.
                  </span>
                </label>
              </div>
            </div>
