// AdminArtists.tsx — Artist Management with instant optimistic UI updates
// Fixes: approve/reject/delete refresh, drawer close after action, counter sync
...
const TAB_CFG: Record<Tab, { label: string; icon: React.ElementType; color: string }> = {
  ...
  approved: { label: 'Approved', icon: UserCheck, color: 'text-emerald-600' },
  rejected: { label: 'Rejected', icon: XCircle,   color: 'text-red-600'     },
};

const REJECT_REASONS = [
  'Invalid Aadhaar Card', 'Invalid Government ID', 'Blurry / Unclear Selfie',
  'Selfie Does Not Match Documents', 'Fake Portfolio', 'Duplicate Account',
  'Incomplete Information', 'Inappropriate Content', 'Other',
];
...
export default function AdminArtists() {
  ...
  // Ref to track current tab in realtime callback (avoids stale closure)
  ...
  // ── Real-time: re-fetch silently when DB changes ──────────────────────────
  useEffect(() => {
  ...
  // ── Optimistic removal: immediately removes artist from local list ─────────
  const removeFromList = useCallback((id: string) => {
    setArtists(prev => prev.filter(a => a.id !== id));
    setTotal(prev => Math.max(0, prev - 1));
  }, []);

  // ── Filtered list (search scoped to current tab) ──────────────────────────
  const filtered = artists.filter(a =>
    !search || [a.full_name, a.email, a.phone, a.profession, a.city]
  ...
  const handleApprove = async (artist: Artist) => {
    ...
    if (result.success) {
      toast.success('Artist approved — profile is now live!');
      setSelected(null);
      await load();
      await loadCounts();
    } else {
  ...
  const handleDelete = async (artist: Artist) => {
    ...
    // Optimistic: remove immediately, close drawer
    removeFromList(artist.id);
    setSelected(null);
    setCounts(c => ({
      ...c,
      pending:  Math.max(0, c.pending  - (artist.verification_status === 'pending'  ? 1 : 0)),
      approved: Math.max(0, c.approved - (artist.verification_status === 'approved' ? 1 : 0)),
      rejected: Math.max(0, c.rejected - (artist.verification_status === 'rejected' ? 1 : 0)),
    }));

    const { error } = await supabase.from('provider_profiles').delete().eq('id', artist.id);
    if (error) { toast.error(error.message); load(); loadCounts(); return; }
  ...
  const exportCSV = () => {
    ...
    el.download = `artists_${tab}.csv`; el.click();
  };
  ...
  return (
    <div className="p-6 space-y-5">
      ...
      <div className="flex gap-1 p-1 bg-secondary rounded-xl w-fit border border-border/50">
        {(Object.entries(TAB_CFG) as [Tab, typeof TAB_CFG[Tab]][]).map(([key, cfg]) => (
          <button key={key} onClick={() => { setTab(key); setPage(0); setSearch(''); }}
            className={cn('flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-semibold transition-all',
              tab === key ? 'bg-white dark:bg-gray-900 shadow-xs text-foreground' : 'text-muted-foreground hover:text-foreground')}>
      ...
      {tab === 'pending' && counts.pending > 0 && (
        <div className="flex items-center gap-3 p-4 bg-amber-50 dark:bg-amber-950/20 border border-amber-200 rounded-2xl">
          ...
          <p className="text-sm font-semibold text-amber-700">
            {counts.pending} artist{counts.pending > 1 ? 's' : ''} waiting for verification. Review each profile carefully before approving.
          </p>
        </div>
      )}
              ...
              {loading ? (
                ...
                <tr key={a.id} className="border-b border-border/40 hover:bg-surface-2 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2.5">
                      {a.avatar_url
                        ...
                        : <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-rose-500 to-red-700 flex items-center justify-center flex-shrink-0">
                  ...
                  <td className="px-4 py-3 text-[10px] text-muted-foreground">
                    ...
                    {a.verified_at && <p className="text-[9px] text-emerald-600 mt-0.5">✓ {new Date(a.verified_at).toLocaleDateString('en-IN')}</p>}
                  </td>
                  <td className="px-4 py-3">
                    <span className={statusBadge(a.verification_status)}>{a.verification_status}</span>
                    {a.rejection_reason && (
                      <p className="text-[9px] text-red-500 mt-1 max-w-[120px] truncate" title={a.rejection_reason}>{a.rejection_reason}</p>
                    )}
                  </td>
                  <td className="px-4 py-3">
      ...
      {rejectModalFor && (
        <div className="fixed inset-0 z-[60] bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-background rounded-2xl border border-border p-6 w-full max-w-md shadow-2xl space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Rejecting <strong>{rejectModalFor.full_name || 'this artist'}</strong>. Select a reason — the artist will be notified.
            </p>
            <div className="space-y-2">
              {REJECT_REASONS.map(r => (
                <label key={r} className="flex items-center gap-2.5 cursor-pointer group" onClick={() => setRejectReason(r)}>
                  <div className={cn('w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-colors',
            ...
            {rejectReason === 'Other' && (
              <textarea value={rejectOther} onChange={e => setRejectOther(e.target.value)}
                placeholder="Describe the reason…" rows={3} className="input-premium text-sm w-full resize-none" />
            )}
            <div className="flex gap-3 pt-2">
              <button onClick={() => { setRejectModalFor(null); setRejectReason(''); setRejectOther(''); }}
